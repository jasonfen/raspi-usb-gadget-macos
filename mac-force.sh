...
tcpdump -i en7 -n -e -c 30 > "$R"/macfix.cap 2>"$R"/macfix.err &
P=$!
sleep 3
kill -0 $P 2>/dev/null || { echo "!! tcpdump failed to start:"; cat "$R"/macfix.err; exit 1; }
...
echo "=== frames captured: $(grep -c . "$R"/macfix.cap) ==="
cat "$R"/macfix.cap
OUTB=$(grep -c "^[0-9:.]* $SELF >" "$R"/macfix.cap)
echo ""
